...
def render_preview(template: dict, output_path: str):
        ...
        if item.get("type") == "video" and item_id != base_video_id:
            ...
            if item_id in input_map:
                ...
                filter_parts.append(
                    ...
                    f"setpts=PTS+{start_time}/TB{video_node}"
                )
    ...
    for item_id, item in track_items_map.items():
        if item.get("type") == "text":
            ...
            # Add alpha/opacity
            if opacity < 1.0:
                drawtext_params.append(f"alpha={opacity}")
            ...
            if shadow_x != 0 or shadow_y != 0:
                ...
                filter_parts.append(
                    f"{current_node}drawtext={':'.join(shadow_params)}:enable='between(t,{start_time},{end_time})'{shadow_node}"
                )
                # Then overlay text on shadow
                filter_parts.append(
                    f"{shadow_node}drawtext={':'.join(drawtext_params)}:enable='between(t,{start_time},{end_time})'{text_node}"
                )
            else:
                filter_parts.append(
                    f"{current_node}drawtext={':'.join(drawtext_params)}:enable='between(t,{start_time},{end_time})'{text_node}"
                )
